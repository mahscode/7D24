...
import {Clientes, Productos, Referidos, Pedidos, Usuarios} from './db';
...
export const resolvers = {
    Query:{
        ...
        totalReferidos: (root) => {
            return new Promise((resolve, object) => {
                Referidos.countDocuments({}, (error, count) => {
                    if(error) rejects(error)
                    else resolve(count)
                });
            })
        ...
        obtenerReferidos : (root, {limite, offset}) => {
            return Referidos.find({}).limit(limite).skip(offset)
        },
        obtenerReferido : (root, {id}) => {
            return new Promise((resolve, object) =>{
                Referidos.findById(id, (error, referido) => {
                    if(error) rejects(error)
                    else resolve(referido)
                })
            })
    ...
    Mutation: {
        crearCliente  : (root, {input}) =>{
            const nuevoCliente = new Clientes({
                ...
                telefonoCasa: input.telefonoCasa,
                telefonoTrabajo: input.telefonoTrabajo,
                ciudad: input.ciudad,
                email : input.email,
        ...
        nuevoProducto : (root, {input}) => {
            ...
            //mongo db crea el id que se genera el objeto
        ...
        nuevoReferido:  (root, {input})=> {

            const nuevoReferido = new Referidos({
                nombre: input.nombre,
                apellido: input.apellido,
                telefono: input.telefono,
                afinidad: input.afinidad,
                tipo: input.tipo,
                observacion: input.observacion,
                // idReferido : input.idReferido
            });
            //mongo crea el id
            nuevoReferido.id =nuevoReferido._id;
            return new Promise((resolve, object) => {
                nuevoReferido.save((error) =>{
                    if(error) rejects(error);
                    else  resolve(nuevoReferido)
                })

            });
        },
    
        actualizarReferido : (root, {input})=>{
        return new Promise((resolve, referido) => {
            Referidos.findOneAndUpdate({_id : input.id}, input, {new: true}, (error, referido)=>{
                if(error) rejects(error);
                else  resolve(referido)
            })
        })
    },
    eliminarReferido : (root, {id})=>{
        return new Promise((resolve, referido) => {
            Referidos.findOneAndDelete({_id : id}, (error) =>{
                if(error) rejects(error);
                else resolve("El Referido se Eliminó correctamente") 
            })
        })
    },
    ...
    actualizarEstado : (root, {input}) => {
        return new Promise ((resolve, object) =>{
            ...
            //Recorrer y actualizar cantidad de productos en base al estado del pedido
